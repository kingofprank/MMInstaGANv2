...
class MMInstaGANModel(BaseModel):
	def name(self):
		return 'MMInstaGANModel'
	...
	def initialize(self, opt):
		...
		#self.loss_names = ['D_A2B', 'G_A', 'cls_A', 'cyc_A', 'ctx_A', 'D_B2A', 'G_B', 'cls_B', 'cyc_B', 'ctx_B']
		self.loss_names = ['D_A2B', 'G_A', 'cls_A', 'cyc_A', 'idt_A', 'ctx_A', 'D_B2A', 'G_B', 'cls_B', 'cyc_B',
						   'idt_B', 'ctx_B']

		# specify the images you want to save/display. The program will call base_model.get_current_visuals
		# visual_names_A_img = ['real_A_img', 'fake_B_img']
		# visual_names_B_img = ['real_B_img', 'fake_A_img']
		# visual_names_A_seg = ['real_A_seg', 'fake_B_seg']
		# visual_names_B_seg = ['real_B_seg', 'fake_A_seg']
		...
		self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.cat_num, opt.ngf, opt.netG, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
		if self.isTrain:
			use_sigmoid = opt.no_lsgan
			self.netD = networks.define_D(opt.output_nc, opt.cat_num, opt.ndf, opt.netD, opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
		...
		if self.isTrain:
			...
			self.optimizer_G = torch.optim.Adam(filter(lambda p: p.requires_grad, itertools.chain(self.netG.parameters())), lr=opt.lr, betas=(opt.beta1, 0.999))
			self.optimizer_D = torch.optim.Adam(filter(lambda p: p.requires_grad, itertools.chain(self.netD.parameters())), lr=opt.lr, betas=(opt.beta1, 0.999))
			self.optimizers = []
	...
	def merge_masks(self, segs):
		...
		return ret.clamp(max=1, min=0) * 2 - 1 # [0,1] -> [-1,1]
	...
	def set_input(self, input):
		self.imgs_list, self.segs_list, self.merge_seg_list = list(), list(), list()
		for i in range(self.cat_num):
			self.imgs_list.append(input['imgs_list'][i].to(self.device))
			self.segs_list.append(self.select_masks(input['segs_list'][i]).to(self.device))
			self.merge_seg_list.append(self.merge_masks(self.segs_list[i]))
	...
	def forward(self, a_cat_idx, b_cat_idx, seg_idx):
		N = self.opt.ins_per
		self.real_A_seg_sng = self.real_A_segs[:, N * seg_idx:N * (seg_idx + 1), :, :]  # ith mask
		self.real_B_seg_sng = self.real_B_segs[:, N * seg_idx:N * (seg_idx + 1), :, :]  # ith mask
		empty = -torch.ones(self.real_A_seg_sng.size()).to(self.device)  # empty image

		self.forward_A = (self.real_A_seg_sng + 1).sum() > 0  # check if there are remaining instances
		self.forward_B = (self.real_B_seg_sng + 1).sum() > 0  # check if there are remaining instances

		a_cat_label = self.idx2onehot(a_cat_idx)
		b_cat_label = self.idx2onehot(b_cat_idx)

		# forward A
		if self.forward_A:
			self.real_A_sng = torch.cat([self.real_A_img_sng, self.real_A_seg_sng], dim=1)
			self.fake_B_sng = self.netG(self.real_A_sng, b_cat_label)
			self.rec_A_sng = self.netG(self.fake_B_sng, a_cat_label)

			self.fake_B_img_sng, self.fake_B_seg_sng = self.split(self.fake_B_sng)
			self.rec_A_img_sng, self.rec_A_seg_sng = self.split(self.rec_A_sng)
			fake_B_seg_list = self.fake_B_seg_list + [self.fake_B_seg_sng]  # not detach
			for i in range(self.ins_iter - seg_idx - 1):
				fake_B_seg_list.append(empty)
		...
		if self.forward_B:
			self.real_B_sng = torch.cat([self.real_B_img_sng, self.real_B_seg_sng], dim=1)
			self.fake_A_sng = self.netG(self.real_B_sng, a_cat_label)
			self.rec_B_sng = self.netG(self.fake_A_sng, b_cat_label)
	...
	def backward_G(self, a_cat_idx, b_cat_idx):
		lambda_A = self.opt.lambda_A
		lambda_B = self.opt.lambda_B
		lambda_idt = self.opt.lambda_idt
		lambda_ctx = self.opt.lambda_ctx

		a_label = torch.LongTensor([a_cat_idx]).to(self.device)
		b_label = torch.LongTensor([b_cat_idx]).to(self.device)
		a_cat_label = self.idx2onehot(a_cat_idx)
		b_cat_label = self.idx2onehot(b_cat_idx)
		# backward A
		if self.forward_A:
			pred_fake_B, cls_fake_B = self.netD(self.fake_B_mul)
			self.loss_G_A = self.criterionGAN(pred_fake_B, True)
			self.loss_cls_A = self.criterionCls(cls_fake_B, b_label)
			self.loss_cyc_A = self.criterionCyc(self.rec_A_sng, self.real_A_sng) * 1
			self.loss_idt_B = self.criterionIdt(self.netG(self.real_A_sng, a_cat_label), self.real_A_sng.detach()) * 1
			weight_A = self.get_weight_for_ctx(self.real_A_seg_sng, self.fake_B_seg_sng)
			self.loss_ctx_A = self.weighted_L1_loss(self.real_A_img_sng, self.fake_B_img_sng, weight=weight_A) * 10
		else:
			self.loss_G_A = 0
			self.loss_cls_A = 0
			self.loss_cyc_A = 0
		...
		# backward B
		if self.forward_B:
			pred_fake_A, cls_fake_A = self.netD(self.fake_A_mul)
			self.loss_G_B = self.criterionGAN(pred_fake_A, True)
			self.loss_cls_B = self.criterionCls(cls_fake_A, a_label)
			self.loss_cyc_B = self.criterionCyc(self.rec_B_sng, self.real_B_sng) * 1
			self.loss_idt_A = self.criterionIdt(self.netG(self.real_B_sng, b_cat_label), self.real_B_sng.detach()) * 1
			weight_B = self.get_weight_for_ctx(self.real_B_seg_sng, self.fake_A_seg_sng)
			self.loss_ctx_B = self.weighted_L1_loss(self.real_B_img_sng, self.fake_A_img_sng, weight=weight_B) * 10
		else:
			self.loss_G_B = 0
			self.loss_cls_B = 0
			self.loss_cyc_B = 0
		...
		# combined loss
		#self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cls_A + self.loss_cls_B + self.loss_cyc_A + self.loss_cyc_B + self.loss_ctx_A + self.loss_ctx_B
		self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cyc_A + self.loss_cyc_B + self.loss_idt_A + self.loss_idt_B + self.loss_ctx_A + self.loss_ctx_B + self.loss_cls_A + self.loss_cls_B
		self.loss_G.backward()


	def backward_D(self, a_cat_idx, b_cat_idx):
		a_label = torch.LongTensor([a_cat_idx]).to(self.device)
		b_label = torch.LongTensor([b_cat_idx]).to(self.device)
		self.loss_D_A2B = self.backward_D_basic(self.real_B, self.fake_B_mul, b_label)
		self.loss_D_B2A = self.backward_D_basic(self.real_A, self.fake_A_mul, a_label)
		self.loss_D = self.loss_D_A2B + self.loss_D_B2A
		self.loss_D.backward()

	def backward_D_basic(self, real, fake, idx):
		# Real
		pred_real, cls_real = self.netD(real)
		loss_D_pred_real = self.criterionGAN(pred_real, True)
		loss_D_cls_real = self.criterionCls(cls_real, idx)
		# Fake
		pred_fake, cls_fake = self.netD(fake.detach())
		loss_D_pred_fake = self.criterionGAN(pred_fake, False)
		loss_D_cls_fake = self.criterionCls(cls_fake, idx)
		# Combined loss
		loss_D = (loss_D_pred_real + loss_D_pred_fake) * 0.5 + (loss_D_cls_real + loss_D_cls_fake) * 1
		# backward

		return loss_D

	def idx2onehot(self, idx):
		return torch.eye(self.cat_num)[[idx], :].detach().to(self.device)

	def optimize_parameters(self):
		for a_cat_idx in range(self.cat_num):
			for b_cat_idx in range(self.cat_num):
				if a_cat_idx >= b_cat_idx:
					continue
				self.real_A_img = self.imgs_list[a_cat_idx]
				self.real_B_img = self.imgs_list[b_cat_idx]
				self.real_A_img_sng = self.real_A_img
				self.real_B_img_sng = self.real_B_img
				self.real_A_segs = self.segs_list[a_cat_idx]
				self.real_B_segs = self.segs_list[b_cat_idx]
				self.real_A_seg = self.merge_seg_list[a_cat_idx] # merge
				self.real_B_seg = self.merge_seg_list[b_cat_idx]
				self.real_A = torch.cat([self.real_A_img, self.real_A_segs], dim=1)
				self.real_B = torch.cat([self.real_B_img, self.real_B_segs], dim=1)
				# self.a_cat_label = self.idx2onehot(a_cat_idx)
				# self.b_cat_label = self.idx2onehot(b_cat_idx)
				self.fake_A_seg_list = list()
				self.fake_B_seg_list = list()
				self.rec_A_seg_list = list()
				self.rec_B_seg_list = list()
				...
				for i in range(self.ins_iter):
					self.forward(a_cat_idx, b_cat_idx, i)
					if self.forward_A or self.forward_B:
						self.set_requires_grad([self.netD], False) # check!
						self.optimizer_G.zero_grad()
						self.backward_G(a_cat_idx, b_cat_idx)
						self.optimizer_G.step()

					if self.forward_A or self.forward_B:
						self.set_requires_grad([self.netD], True)
						self.optimizer_D.zero_grad()
						self.backward_D(a_cat_idx, b_cat_idx)
