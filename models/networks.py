...
def define_G(input_nc, output_nc, cat_num, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    ...
    if netG == 'basic':
        ...
        net = ResnetMMSetGenerator(input_nc, output_nc, cat_num, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netG)
    return init_net(net, init_type, init_gain, gpu_ids)


def define_D(input_nc, cat_num, ndf, netD, n_layers_D=3, norm='batch', use_sigmoid=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    ...
    if netD == 'basic':
        ...
        net = NLayerMMSetDiscriminator(input_nc, cat_num, ndf, n_layers=3, norm_layer=norm_layer, use_sigmoid=use_sigmoid)
    else:
...
class ResnetMMSetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, cat_num=3, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        assert (n_blocks >= 0)
        super(ResnetMMSetGenerator, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        if type(norm_layer) == functools.partial:
        ...
        self.encoder_img = self.get_encoder(input_nc, n_downsampling, ngf, norm_layer, use_dropout, n_blocks, padding_type, use_bias)
        self.encoder_seg = self.get_encoder(1, n_downsampling, ngf, norm_layer, use_dropout, n_blocks, padding_type, use_bias)
        self.decoder_img = self.get_decoder(output_nc, n_downsampling, 2 * ngf, norm_layer, use_dropout, padding_type, use_bias)  # 2*ngf
        self.decoder_seg = self.get_decoder(1, n_downsampling, 3 * ngf, norm_layer, use_dropout, padding_type, use_bias)  # 3*ngf

        self.adain_params_num_img = self.get_num_adain_params(self.decoder_img)
        self.adain_params_num_seg = self.get_num_adain_params(self.decoder_seg)

        self.mlp = MLP(cat_num, (self.adain_params_num_img + self.adain_params_num_seg), dim=256, n_blk=3, norm='none', activ='relu')
    ...
    def get_decoder(self, output_nc, n_downsampling, ngf, norm_layer, use_dropout, padding_type, use_bias):
        model = []
        mult = 2 ** n_downsampling
    ...
    def forward(self, inp, label):
        ...
        adain_params = self.mlp(label)
        img_adain_params = adain_params[:, :self.adain_params_num_img]
        seg_adain_params = adain_params[:, self.adain_params_num_img:]
        self.assign_adain_params(img_adain_params, self.decoder_img)
        self.assign_adain_params(seg_adain_params, self.decoder_seg)
...
class NLayerMMSetDiscriminator(nn.Module):
    
    def __init__(self, input_nc, cat_num, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        super(NLayerMMSetDiscriminator, self).__init__()
        self.input_nc = input_nc
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        self.feature_img = self.get_feature_extractor(input_nc, ndf, n_layers, kw, padw, norm_layer, use_bias)
        self.feature_seg = self.get_feature_extractor(1, ndf, n_layers, kw, padw, norm_layer, use_bias)
        self.discriminator = self.get_discriminator(2 * ndf, n_layers, kw, padw, norm_layer, use_sigmoid)  # 2*ndf
        self.classifier = self.get_classifier()
        self.fc = nn.Linear(256, cat_num)
    ...
    def get_discriminator(self, ndf, n_layers, kw, padw, norm_layer, use_sigmoid):
        nf_mult_prev = min(2 ** (n_layers - 1), 8)
        nf_mult = min(2 ** n_layers, 8)
        model = [
            # Use spectral normalization
    ...
    def get_classifier(self):
        model = [
            # Use spectral normalization
            SpectralNorm(nn.Conv2d(512, 256, kernel_size=25, stride=1, padding=0)),
            nn.LeakyReLU(0.2, True)
        ]
        return nn.Sequential(*model)
